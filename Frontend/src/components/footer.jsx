...
export default function Footer() {
  const cols = [
    { title: 'Product', links: ['Features', 'Integrations', 'Pricing', 'ROI Calculator', 'Changelog'] },
    { title: 'Company', links: ['About', 'Customers', 'Careers', 'Press', 'Contact'] },
    { title: 'Resources', links: ['Blog', 'Help center', 'API docs', 'Security', 'Status'] },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-10">
        <div className="bg-white/5 border border-white/10 rounded-3xl p-9 lg:p-14 mb-16 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          ...
          <div className="flex gap-2">
            ...
            <button className="pill-btn px-6 py-3 text-sm font-semibold border border-white/30 text-white hover:bg-white/10">Book a demo</button>
          </div>
        </div>
          ...
          {cols.map((c) => (
            <div key={c.title}>
              ...
              <ul className="space-y-3">
                {c.links.map((l) => (
                  <li key={l}><a href="#" className="text-sm text-white/85 hover:text-white">{l}</a></li>
                ))}
              </ul>
            </div>
          ))}
